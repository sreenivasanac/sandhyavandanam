...
- **Audio (chant/pronunciation)**: laukika items (no Vedic svaras, no personalised placeholders — 134 unique clips) are AI-chanted with **Vāgdhenu** (Apache-2.0), rendered locally on the M4 Pro via MPS (`docs/reference/tools/VAGDHENU_LOCAL.md`; ~1 h batch, no GPU rental — options in `GPU_OPTIONS.md` if ever needed), converted to AAC (`app/public/audio/*.m4a`, ~56 kbps, loudness-normalised) and wired by `scripts/audio_shard.py wire`. UI: a ▶ Chant button per mantra, labelled as AI-synthesised; toggle in Settings; clips are runtime-cached by the service worker for offline. Vedic (svara) mantras have no audio — no TTS honours svaras; human recordings remain the plan there. Research: `docs/reference/tools/CHANT_AUDIO.md`.
